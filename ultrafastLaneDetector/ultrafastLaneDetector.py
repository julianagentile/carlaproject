	...
	def initialize_model(model_path, cfg, use_gpu):
		...
		if use_gpu:
			if torch.backends.mps.is_built():
				...
				net = net.cuda()
				state_dict = torch.load(model_path, map_location='cuda', weights_only=False)['model'] # CUDA - Fixed for PyTorch 2.6
		else:
			state_dict = torch.load(model_path, map_location='cpu', weights_only=False)['model'] # CPU - Fixed for PyTorch 2.6
